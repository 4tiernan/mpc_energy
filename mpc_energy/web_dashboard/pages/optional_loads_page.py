    ...
    with tabs[idx + 1]:
        ...
        if row["load_type"] == "ev":
            ...
            if row["charger_model"] == "Tesla API":
                ...
                c_t4, c_t5 = st.columns(2)
                row["charge_current_entity_id"] = c_t4.text_input("Charge Current Entity ID", value=row.get("charge_current_entity_id", ""), key=f"ev_t_cur_ent_{idx}")
                row["charge_enable_entity_id"] = c_t5.text_input("Charge Enable Switch Entity ID (Fallback)", value=row.get("charge_enable_entity_id", ""), key=f"ev_t_en_ent_{idx}", help="Used for older versions of the Sigenergy integration that control the charger run state via a switch entity.")
                c_btn1, c_btn2 = st.columns(2)
                row["charge_start_entity_id"] = c_btn1.text_input("Charge Start Button Entity ID", value=row.get("charge_start_entity_id", ""), key=f"ev_t_start_ent_{idx}", help="Used for newer versions of the Sigenergy integration that start charging via a button entity.")
                row["charge_stop_entity_id"] = c_btn2.text_input("Charge Stop Button Entity ID", value=row.get("charge_stop_entity_id", ""), key=f"ev_t_stop_ent_{idx}", help="Used for newer versions of the Sigenergy integration that stop charging via a button entity.")
                c_t6, c_t7 = st.columns(2)
                row["power_entity_id"] = c_t6.text_input("Charger Power Entity ID (kW)", value=row.get("power_entity_id", ""), key=f"ev_pent_{idx}")
